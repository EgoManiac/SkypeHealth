function Get-CbSkypeHealth {
  ...
  <#
    .SYNOPSIS
    Simple Cmdlet to get various items of the health of your Skype Farm and output it to a HTML File

    .DESCRIPTION
    This cmdlet is designed to be a simple entry point to verifying the health of your environment. By running the caoomand the Script will pull all of the generic health monitors 
    and display this in a simple self contained HTML file. The HTML file has embedded JavaScript and CSS to try and make the report "C Level" Appropriate.

    This script provides a very high level overview and should be used as a starting point for your troubleshooting.

    .EXAMPLE
    Get-CbSkypeHealth -UserLogo https://upload.wikimedia.org/wikipedia/commons/8/86/Microsoft_Skype_for_Business_logo.png
    This command overrides the builtin logo to allow you to specify a custom company logo. The URL must be accessible from the MAchne running this cmdlet.

    .NOTES
    Colours are unfortunately set within the code, but future releases may use an object to format it.
  ...
  [cmdletbinding()]
   Param (
    # Unused at this moment in time
    [Parameter(Position=0,ValueFromPipeline=$True)]
    $Server,
    # CSS location - please enter the location of the css code to format your table - Will be used in a future version
    ...
    # Federation Partner List - An array of domains that you would like the report to check. - Currently using dell.com, gcicom.net, hpe.com
    [Parameter(Position=3)]
    [String[]]
    $FederationPartners = @('dell.com','gcicom.net','hpe.com')

  )

    #region Variables
    $CompanyLogoBase64 = (Get-CbImageBase64 -Url $UserLogo)
    $colorscheme = '#222222'
    $brand2color = '#425563'
...
"@
    ...
    #region functions
    function Confirm-ServerCSService {
    [cmdletbinding()]
    Param (
        [Parameter(Position=0,ValueFromPipeline=$True)]
        $Server
    ...
    Process
    {
        ...
        $TableReturn = $csw | Select-Object Name,Status | ConvertTo-Html -Fragment
        $Return = @{
        'Result' = $NoError;
    ...
    function Confirm-ServerResponds {
    [cmdletbinding()]
    Param (
        [Parameter(Position=0,ValueFromPipeline=$True)]
        $Server
    ...
    function Get-StatusTable($ServerList,$pingonly) {
    $i=0
    Foreach ($PoolIdentity in $ServerList.Identity) {
        ...
        if ($HashCheckedComputers.ContainsKey($_)) {
            ...
            if ($pingonly) {
            $ServerOK = $_ | Confirm-ServerResponds
            If ($ServerOK) { 
                $ComputerTDclass = 'computer_pass'
            } else {
                $ComputerTDclass = 'computer_fail'
            ...
            $ServerServiceOK = $_ | Confirm-ServerCSService
            If ($ServerServiceOK.result) { 
                $ComputerTDclass = 'computer_pass'
            } else {
                $ComputerTDclass = 'computer_fail'
    ...
    function Confirm-ReplicationStatus($ReplicaFqdn)
    {
    ...
    function Confirm-ServicesAreRunning{
    [cmdletbinding()]
    Param (
        [Parameter(Position=0,ValueFromPipeline=$True)]
        $Server
    ...
    function Confirm-CSDatabaseServicesAreRunning{
    [cmdletbinding()]
    Param (
        [Parameter(Position=0,ValueFromPipeline=$True)]
        $PoolIdentity
    ...
    foreach ($dbpool in $objDatabaseServers.identity){
        $ServerOK = $dbpool | Confirm-CSDatabaseServicesAreRunning
        If ($ServerOK) { 
        $ComputerTDclass = 'computer_pass'
        } else {
        $ComputerTDclass = 'computer_fail'
    ...
    function Get-CSAddressBookTable(){
    ...
    function Get-FederationPartnerTable(){
    ...
    $FedServer = ((Get-CsService -EdgeServer) | where-object {$_.serviceid -eq $FedService}).Poolfqdn
...
"@
    ...
    $PoolsTableWithAttib = ($pools | Select-Object Identity,@{Name='Services'
    ...
    $strCsWindowsServiceHTML += Get-StatusTable $objRegistrarServers $false
    ...
    $strCsWindowsServiceHTML += Get-StatusTable $objMediationServers $false
    $strCsWindowsServiceHTML += '</div></div>'
    ...
    $strCsWindowsServiceHTML += Get-StatusTable $objEdgeServers $true 
    $strCsWindowsServiceHTML += '</div></div>'
    ...
    $strCsWindowsServiceHTML += Get-StatusTable $objDatabaseServers $true 
    ...
    $strReplicationHTML += Get-DatabaseTable
    $strReplicationHTML += '</div></div>'
    ...
    $strHealthHTML += Get-CSAddressBookTable
    $strHealthHTML += '</div></div>'
    ...
    $strHealthHTML += Get-FederationPartnerTable
    $strHealthHTML += '</div></div>'
...
function Get-CBUserCounts {
    ...
    $PoolCount = $EntirePoolCount | Group-Object RegistrarPool | Select-object Count,Name | Sort-Object Count -Descending
    $EVUsers = ($EntirePoolCount | where-object {$_.EnterpriseVoiceEnabled -eq $true}).count
    $ExUMUsers = ($EntirePoolCount | where-object {$_.ExUMEnabled -eq $true}).count
...
function Get-CbImageBase64 {
    ...
    <#
      ...
      .EXAMPLE
      Get-CbImageBase64 -url http://www.google.com/logo.jpg
